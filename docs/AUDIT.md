...
## 6. 🟠 Sécurité & confidentialité — _« le repo public, est-ce grave ? »_
...
- 🟡 **Propriété / réutilisation** : sans `LICENSE`, le droit d'auteur par défaut s'applique
  ...
  (ex. code sous MIT + contenu sous _All Rights Reserved_ ou Creative Commons BY-NC).
...
**P1 — court terme** 3. `npm run validate` (intégrité données) **en CI**. 4. Compléter/retirer proprement `role="grid"` (sémantique a11y). 5. ESLint + Prettier ; page **404**. 6. Sources/références par fiche (crédibilité éditoriale).

**P2 — moyen terme** 7. Recherche **insensible aux accents** + tolérante aux fautes. 8. **JSON-LD** (données structurées). 9. Lighthouse de référence ; mode sombre ; rééquilibrage du contenu des familles pauvres.
